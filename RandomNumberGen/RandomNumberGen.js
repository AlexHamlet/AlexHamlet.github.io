    ...
    if (key === 13) {
        if (document.getElementById("int").checked) {
            if (document.getElementById("low").value < document.getElementById("high").value) {
                var low = Math.ceil(document.getElementById("low").value);
                var high = Math.floor(document.getElementById("high").value);
            }
            else {
                var high = Math.floor(document.getElementById("low").value);
                var low = Math.ceil(document.getElementById("high").value);
            }
            ...
            for (var i = 0; i < many; i++) {
                if (i == 0) {
                    document.getElementById("numbers").innerHTML += " " + Math.floor((Math.random() * diff + parseFloat(low)) * 100) / 100;
                }
                else {
                    document.getElementById("numbers").innerHTML += ", " + Math.floor((Math.random() * diff + parseFloat(low)) * 100) / 100;
                }
